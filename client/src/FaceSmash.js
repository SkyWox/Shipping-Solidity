...
var GeektABI = [
  ...
  {
    constant: true,
    inputs: [{ name: 'SHA256notaryHash', type: 'bytes32' }],
    name: 'getImage',
  ...
  },
  {
    ...
    inputs: [
      ...
      { name: 'SHA256notaryHash', type: 'bytes32' }
    ],
    name: 'addImageToUser',
    outputs: [{ name: 'success', type: 'bool' }],
    payable: false,
  ...
  {
    constant: false,
    inputs: [{ name: 'badUser', type: 'address' }],
    name: 'removeUser',
    outputs: [{ name: 'success', type: 'bool' }],
    payable: false,
    type: 'function'
  ...
  {
    constant: false,
    inputs: [{ name: 'badImage', type: 'bytes32' }],
    name: 'removeImage',
    outputs: [{ name: 'success', type: 'bool' }],
    payable: false,
    type: 'function'
...
class FaceSmash extends Component {
        ...
        if (typeof web3.eth.accounts[0] !== 'undefined') {
          ...
          GeektContract.getUsers(function(err, usersResult) {
            if (err) {
              ...
              console.log(
                'saw users: ' + JSON.stringify(usersResult) + ', err : ' + err
              )
              var usersList = {}
                ...
                GeektContract.getUser(key, function(err, userResult) {
                  ...
                  outerThis.setState(
                    ...
                    function() {
                      ...
                      if (
                        Object.keys(outerThis.state.users).length ===
                        usersLength
                      ) {
                        // this is sort of a funky callback mechanism
  ...
  getSelected() {
    // draws the selected user profile in the bottom left panel
    var outerThis = this
    //console.log("selectedUser : " + outerThis.state.selectedUser);
    //console.log("users : " + JSON.stringify(outerThis.state.users));
    if (
      outerThis.state.selectedUser &&
      outerThis.state.users[outerThis.state.selectedUser]
    ) {
      return (
        <span>
          ...
          <span style={{ fontSize: '13px', fontWeight: 'bold' }}>
            {outerThis.state.selectedUserImageSHA}{' '}
            {outerThis.selectedUserCheckMark()}
          </span>
          <br />
          <br />
          Image Timestamp:{' '}
          <span style={{ fontSize: '13px', fontWeight: 'bold' }}>
            {outerThis.selectedUserTimestamp()}
          </span>
          <br />
          <br />
          User Images:{' '}
          {outerThis.drawImageSelect(0, outerThis.state.selectedUser)}
        </span>
      )
    } else {
      return null
      ...
      return (
        <span>
          {this.formatTimestamp(
            this.state.selectedUserImages[this.state.selectedUserImage]
              .timestamp
          )}
        </span>
      )
  ...
  getDefault(attribute) {
    // overloaded function for drawing input boxes, to reduce code repitition...
    var outerThis = this
    if (defaultAccount === 0) {
      // we didn't see a "defaultAccount" loaded, maybe no web3 connection (they can't update a profile yet)
      return (
        <span>
          {attribute}:{' '}
          <input
      ...
      // acct exists and user has already signed in (they can't update the profile, we can fill in their account details)
      if (
        outerThis.state.thisUser &&
        outerThis.state.users[outerThis.state.thisUser]
      ) {
        if (attribute === 'Handle') {
          ...
          return (
            <h5 style={{ display: 'inline-block' }}>
              {outerThis.state.users[outerThis.state.thisUser][attribute]}
              {includeComma}
            </h5>
          )
        }
      } else {
        // acct exists but user has yet to sign in (they can sign the GuestBook now!)
        return (
          <span>
            {attribute}:{' '}
            <input
  ...
  refreshUser(key) {
    // make sure we always have up-to-date user profile info when selectbox or user update callbacks fire
        ...
        if (typeof web3.eth.accounts[0] !== 'undefined') {
          ...
          GeektContract.getUser(key, function(err, userResult) {
            ...
            GeektContract.getUserImages(key, function(err, imagesResult) {
              if (err) {
                console.log(
                  'getUserImages saw err for user ' + key + ': ' + err
                )
              } else {
                console.log(
                  'saw user images : ' +
                ...
                _.map(imagesResult, function(imageHash) {
                  GeektContract.getImage(imageHash, function(err, imageResult) {
                    if (key === defaultAccount) {
                      ...
                      console.log(
                        'saw image ' +
                          ...
                          ', myNewImages.length : ' +
                          Object.keys(newMyImages).length +
                          ', length :' +
                          length
                      )
                      if (Object.keys(newMyImages).length === length) {
                        console.log(
                          'all myImages loaded, myImages length : ' +
                            length +
                            ', saw latestImage: ' +
                            latestImage
                        )
                    ...
                    newSelectedImages[imageHash].timestamp = parseInt(
                      imageResult[1]
                    )
                    if (
                      newSelectedImages[imageHash].timestamp >
                      selectHighestTimestamp
                    ) {
                      selectHighestTimestamp =
                        newSelectedImages[imageHash].timestamp
                    ...
                    console.log(
                      ' - saw image ' +
                        ...
                        Object.keys(newSelectedImages).length +
                        ', length :' +
                        length
                    )
  ...
  newUserSelected(e) {
    // handles onChange of the userSelect box
  ...
  drawUserSelect() {
    // draws the userSelect box and account links, in alphabetical order
    var outerThis = this
    ...
    return (
      <select
        ...
        ref="userSelect"
        style={{ width: '300px' }}
        onChange={outerThis.newUserSelected}
      >
        <option disabled>Select user</option>
        {_.sortBy(
          _.map(this.state.users, function(val, key) {
            // sort users by handle alphabetically here
            //console.log("key("+ key +"), val['addres'] : " + val['address']);
            return (
              <option value={key} key={val['address']}>
                {key}
              </option>
            ) // don't mind the funky "reverse" key/val nomenclature here...
          }),
          function(i) {
  ...
  viewOrSignButton() {
    // users will see a "sign now", or "add new image" input/button, or nothing if web3 didn't load properly
    var outerThis = this
    if (defaultAccount === 0) {
      return null
    } else {
      if (outerThis.state.thisUser) {
        // acct exists and user has already signed in, they can now add images!
        return (
          <span>
            <div
              style={{
  ...
  goBack() {
    // puts the bottom left panel back into ready state for new images to be added, after an image is added
  ...
  newImageButton() {
    // display SHA256 of the image to be added, and the button to click to add it to this user's account on-chain
    ...
    if (outerThis.state.newImageReady === 1) {
      return (
        <span>
          ...
          <br />
          <br />
          <span style={{ color: 'blue' }} onClick={() => outerThis.goBack()}>
            Go Back
          </span>
        </span>
      )
    } else if (outerThis.state.newImageReady) {
      // if newImageReady is not 0/1, it's being used for loading / error display message
      return <span>{outerThis.state.newImageReady}</span>
    } else {
      // this is the default "ready to add new image" state, showing the myImages selector and the "paste image URL here" input box
      return (
        <span>
          Image SHA:{' '}
          <span style={{ fontSize: '13px', fontWeight: 'bold' }}>
            {outerThis.state.newImageSHA} {outerThis.myUserCheckMark()}
          </span>
          <br />
          <br />
          Your Images: {outerThis.drawImageSelect(1, this.state.thisUser)}
          <br />
          <br />
          <span>
            Add Image:{' '}
            <input
              ...
              onChange={this.RegisterChange}
              onFocus={() => {
                this.clearStateIf('newImageURL', 'Paste Image URL Here', null)
              }}
            />
            <br />
            <br />
  ...
  selectedUserImageSelected(e) {
    console.log(
      'selectedUserImageSelected : name(' +
        e.target.name +
        '), value(' +
        e.target.value +
        ')'
    ...
    if (thisUser && outerThis.state.users[thisUser]) {
      ...
      if (mine) {
        ...
        return (
          <select
            id="myImagesSelect"
            className="myImagesSelect"
            ref="myImagesSelect"
            style={{ width: '300px' }}
            onChange={outerThis.newMyImageSelected}
          >
            <option disabled>Your Images: </option>
            {_.sortBy(
              _.map(outerThis.state.myImages, function(val, hash) {
                // sort users by handle alphabetically here
                ...
                return (
                  <option value={hash} key={val['timestamp']}>
                    {thisImageString}
                  </option>
                ) // don't mind the funky "reverse" key/val nomenclature here...
              }),
        ...
        return (
          <select
            id="selectedUserImagesSelect"
            className="selectedUserImagesSelect"
            ref="selectedUserImagesSelect"
            style={{ width: '300px' }}
            onChange={outerThis.selectedUserImageSelected}
          >
            <option disabled>User Images: </option>
            {_.sortBy(
              _.map(outerThis.state.selectedUserImages, function(val, hash) {
                // sort users by handle alphabetically here
                //console.log("hash("+ hash +"), val : " + JSON.stringify(val));
                var thisImageSplit = val['url'].split('/')
                var thisImageString = val['ur']
  ...
  addImageToUser() {
    // handles the "add image to user" button functionality
    var outerThis = this
    console.log('adding image to user now!')
    console.log(
      'imageURL : ' +
        outerThis.state.newImageURL +
        ', sha: 0x' +
        outerThis.state.newImageSHA
    )
    try {
      GeektContract.addImageToUser.estimateGas(
        outerThis.state.newImageURL,
        '0x' + outerThis.state.newImageSHA,
        { from: defaultAccount },
        function(err, result) {
          if (err) {
            throw err
            ...
            var myGasNum = result
            GeektContract.addImageToUser.sendTransaction(
              outerThis.state.newImageURL,
              '0x' + outerThis.state.newImageSHA,
  ...
  signGuestbook() {
    // handles the "sign guestbook now" button functionality
    var outerThis = this
  ...
  RegisterChange(e) {
    // updates input fields in response to user typing in them, gets SHA256 of new image if needed
      ...
      fetch(request)
        ...
        .then(function(respBlob) {
          ...
          reader.onloadend = function() {
            ...
            var sha256 = cryptojs
              .SHA256(cryptojs.enc.Latin1.parse(dataURL))
  ...
  fetchNewImage() {
    // when URL is pasted into newImage box, simple error-check, display, and get SHA256 hash of the image also
    var outerThis = this
    console.log('fetch new image : ' + this.state.newImageURL)
    var lastFour = this.state.newImageURL.substr(
      this.state.newImageURL.length - 4
    )
    var isNewImage = /png|bmp|gif|jpg|jpeg|tif|tiff/.test(
    ...
    var myImage = this.refs.myImageRef
    var proxyURL =
      'https://fierce-temple-74228.herokuapp.com/' + this.state.newImageURL
    if (isNewImage) {
      outerThis.setState({
  ...
  clearState(target, warn) {
    // clears input boxes from their default value upon user clicking on that input box
  ...
  clearStateIf(target, val, warn) {
    // nested clearState calls allow for custom error/warning message clearing also, if needed
    console.log(
      'clearStateIf called! target(' +
        this.state[target] +
        '), val (' +
        val +
        ')'
  ...
  componentDidMount() {
    // gets called every time a redraw occurs on the main top-level react elect (so, all the time)
    var outerThis = this
    if (!alreadyLoaded) {
  ...
  render() {
    // renders the main page, updates automatically upon setState() calls which alter the state
    return (
      <div className="App">
        <div className="App-header">
          <table
            style={{ minWidth: '70%', textAlign: 'center', margin: 'auto' }}
          >
            <tbody>
              <tr>
                <td style={{ width: '200px' }}>
                  <a href="http://www.enledger.io/" target="_blank">
                    <img
                ...
                <td style={{ width: '250px' }}>
                  See the BlockGeeks Article<br />
                  <br />
                  See the{' '}
                  <a
                    href="https://github.com/Tectract/ethereum-demo-tools"
                    target="_blank"
                  >
        ...
        <div className="App-main">
            ...
            <tbody style={{ height: '100%' }}>
              ...
              <tr className="bookMain">
                ...
                <td style={{ width: '50%', border: '1px solid black' }}>
                  ...
                  <br />
                  Saw default account: {this.defaultEthAddressLink()}
                  <br />
                  <br />
          ...
          <span className="app-outro">
            <table
              style={{ minWidth: '70%', textAlign: 'left', margin: 'auto' }}
            >
              <tbody>
                <tr>
                ...
                <tr>
                  <td>
                    <span style={{ fontSize: '13px', fontWeight: 'bold' }}>
                      To use this tool you&#39;ll need a connection to an
                      Ethereum network, via:<br />
                      <span style={{ padding: '0px 0px 0px 6px' }}>
                        ...
                        running at localhost:8545, then reload this page
                      </span>
                      <br />
                      <span style={{ padding: '0px 0px 0px 6px' }}>
                        2. Install{' '}
                        <a href="https://metamask.io/" target="_blank">
                          Metamask plugin
                        </a>, connect to network of your choice (including
                        Mainnet!), then reload this page
                      </span>
                      <br />
                      <u>notes</u>: for localhost testrpc (testnet), you
                      don&#39;t need Metamask running, see{' '}
                      <a
                        href="https://github.com/Tectract/ethereum-demo-tools/blob/master/README.md"
                        target="_blank"
                      >
                        the README
                      </a>{' '}
                      for metamask signing locally & ethereumjs-testrpc notes<br />
                      <u>notes</u>: sometimes you may need to reload once or
                      twice for it to see your web3.eth.accounts[0] account
                      <br />
                      <br />
                      Author:{' '}
                      <a
                      ...
                      <a href="http://blockgeeks.com/" target="_blank">
                        BlockGeeks.com
                      </a>!<br />
                      Issues, comments, suggestions? Please use{' '}
                      <a
                        href="https://github.com/Tectract/ethereum-demo-tools/issues"
                        target="_blank"
                      >
                        this page
                      </a>{' '}
                      to start an issue ticket, do not email Ryan for help
                      directly :)<br />
                      Also check out{' '}
                      <a
                        href="http://www.enledger.io/EthDeployer/"
                        target="_blank"
                      >
                        Tectract&#39;s EthDeployer!
                      </a>
                    </span>
                  </td>
                </tr>
